...
/**
 * Persistent CTA section with lamp effect, adapted from Aceternity UI.
 * Appears above the footer on every page except /contact.
 * Uses brand-orange for the conic gradient and brand-blue for the glow bar.
 * Text content sits BELOW the lamp lighting cone.
 */
export function LampCTA() {
  ...
  return (
    <section className="relative flex flex-col items-center justify-center overflow-hidden bg-brand-dark w-full z-0 pt-24 pb-20 md:pt-32 md:pb-24">
      {/* Lamp effect — self-contained block */}
      <div className="relative flex w-full scale-y-125 items-center justify-center isolate z-0 h-[280px]">
        <motion.div
        ...
        <div className="absolute inset-auto z-50 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-brand-orange opacity-50 blur-3xl" />
        <motion.div
          initial={{ width: '8rem' }}
          whileInView={{ width: '16rem' }}
          transition={{ delay: 0.3, duration: 0.8, ease: 'easeInOut' }}
          viewport={{ once: true }}
          className="absolute inset-auto z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-brand-orange/80 blur-2xl"
        />
        <motion.div
          initial={{ width: '15rem' }}
          ...
          transition={{ delay: 0.3, duration: 0.8, ease: 'easeInOut' }}
          viewport={{ once: true }}
          className="absolute inset-auto z-50 h-0.5 w-[30rem] -translate-y-[7rem] bg-brand-orange"
        />
        <div className="absolute inset-auto z-40 h-44 w-full -translate-y-[12.5rem] bg-brand-dark" />
      </div>

      {/* Content — sits BELOW the lamp, not overlapping */}
